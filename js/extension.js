(function() {
	class candleTheme extends window.Extension {
		constructor() {
			...
			if(document.getElementById('things').innerHTML == 'No devices yet. Click + to scan for available devices.' || document.getElementById('things').innerHTML == ''){
				...
				try{
				  navigator.brave.isBrave().then(response => {
					...
					//console.log("caught error trying to detect Brave browser: ", err);
				}
			}
			...
			*/
			...
			if(group_remove_view != null){
				group_remove_view.prepend(remove_group_question);
			}
			...
			try{
				...
				*/
				...
				console.error("candle theme: caught error attaching to log add and remove buttons failed?: ", err);
			}
	        ...
	        if( current_path.startsWith('/things') ){
	            ...
	            window.setTimeout(() => {
	                if(window.location.pathname == '/logs'){
	                    if(this.debug){
	                        console.log("candle theme debug: still on the log page 10 seconds later. Saving last log view time.");
	                    }
	                    ...
	                    window.API.postJson(
	                      `/extensions/${this.id}/api/ajax`,
	                        ...
	                        if(this.debug){
	                            console.log("candle theme debug: save_last_logs_view_time response: ", body);
	                        }
		
	                    }).catch((err) => {
	              			if(this.debug){
								console.log("candle theme debug: caught error calling save_last_logs_view_time: ", err);
							}
	                    });	
	                    ...
	                    if(this.debug){
	                        console.log("candle theme debug: logs page was only visited momentarily");
	                    }
	                }
	            }, 10000);
	    ...
	    */
	    ...
	    update_logs_list(){
	        if(this.debug){
	            console.log("candle theme debug: in update_logs_list");
	        }
	        return new Promise((resolve, reject) => {

	            API.getLogs().then((logs) => {
	                if(this.debug){
	                    console.log("candle theme debug: API.getlogs: ", logs);
	                    console.log("candle theme debug: API.getthings: ", this.things);
	                }
	                ...
	                if(this.debug){
	                    console.log("candle theme debug: updated list of devices with logs: ", this.devices_with_logs);
	                }
	        ...
	        window.API.postJson(
	          `/extensions/${this.id}/api/ajax`,
	            ...
	            if(typeof body.last_log_view_time != 'undefined'){
	                if(body.last_log_view_time != null){
	                    if(body.last_log_view_time != "unknown"){
	                        try{
	                            ...
	                            if(this.debug){
									console.error("candle theme debug: caught error adding/updating last_log_view_time_container: ", err);
								}
	                        }
	                    }
	                }   
	  			...
	  			if(this.debug){
					console.error("candle theme debug: caught error calling get_last_logs_view_time: ", err);
				}
	        });	
	    }
	    ...
	    add_link_to_logs(device_id){
	        if(this.debug){
	            console.log("candle theme debug: in add_link_to_logs (which adds a shortcut to the thing's logs in the top right corner)");
	        }
	        device_id = this.get_device_id_from_url(device_id);
	        if( this.devices_with_logs.includes(device_id)){
				const link_to_logs_container_el = document.getElementById('candle-theme-link-to-logs-container');
				const dashboard_addon_view_el = document.getElementById('extension-dashboard-view');
				if(dashboard_addon_view_el){
		            if(this.debug){
						console.log("candle theme debug: not adding link to logs because dashboard addon is enabled.  device_id: ", device_id);
					}
					if(link_to_logs_container_el){
						link_to_logs_container_el.style.display = 'none';
					}
				}
				else{
		            if(this.debug){
						console.log("candle theme debug: device_id has logs, adding direct link to them. device_id: ", device_id);
					}
		            if(link_to_logs_container_el){
						link_to_logs_container_el.style.display = 'block';
					}
					else{
